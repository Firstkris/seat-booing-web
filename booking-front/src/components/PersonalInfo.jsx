...
function PersonalInfo({ setLoading }) {
    const { editUserData, onSelectSchedule } = useAuthContext();
    ...
    const onConfirmDeleteBooking = async () => {
        setLoading(true);
        console.log(warning);
        await onDeleteBooking(warning.booking_detail_id, warning.shedule_id);
        hdlOnClose();
        setTimeout(() => {
            setLoading(false);
    ...
    const departureDate = new Date(warning.departureDate).toLocaleString(
        'th-TH',
        options
    );
    const departTime = new Date(warning.departTime).toLocaleTimeString([], {
        hourCycle: 'h24',
        hour: '2-digit',
        minute: '2-digit',
    });

    const onEditBooking = async (items) => {
        ...
        const currentSchedule = {
            ...
            departTime: items.departTime,
            arrivalTime: items.arrivalTime,
            departureDate: items.departureDate,
            price: items.price,
            seatAvailable: items.seatAvailable,
            status: items.status,
            busId: items.busId,
        };
        await onSelectSchedule(currentSchedule);

        // console.log(items);
        localStorage.setItem(
            'booking_detail_id',
            JSON.stringify({ id: items.booking_detail_id, isEdit: true })
        );
                            ...
                            <div className='flex flex-col gap-4'>
                                ...
                                <p className='text-xl'>
                                    ...
                                    <span className='text-primary font-semibold'>
                                        {' '}
                                        {warning.origin} - {warning.destination}{' '}
                                    </span>
                                    ที่นั่ง{' '}
                                    <span className='text-primary font-semibold'>
                                        {' '}
